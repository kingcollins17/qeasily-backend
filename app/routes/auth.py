...
async def find_user(email: str, db: Annotated[aiomysql.Connection, Depends(get_db)]):
    ...
    if user:
        return User(id=user['id'], user_name=user['user_name'],
...
@route.get("/profile")
async def get_profile(id: int, user: Annotated[User, Depends(get_current_user)], db: Annotated[aiomysql.Connection, Depends(get_db)]):
...
async def has_profile(profile: UserProfile, db: Annotated[aiomysql.Connection, Depends(get_db)]):
